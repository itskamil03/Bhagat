...
/**
 * Return Cloudinary URL or local backend URL for certificate
 */
export function certificatePdfUrl(certificate) {
  ...
  if (url.startsWith("/")) {
    // If it's explicitly a relative path from the root
    return url;
  }
  
  // Local uploads are just bare filenames now — point straight at the backend static folder
  const baseUrl = API_BASE.replace(/\/$/, "");
  return `${baseUrl}/uploads/certificate/${encodeURIComponent(url)}`;
}
